...
VALUES 
  ((SELECT id FROM "USER" WHERE name = '王小明'), 
   (SELECT id FROM "CREDIT_PACKAGE" WHERE name = '14 堂組合包方案'), 14, 2520),
  ((SELECT id FROM "USER" WHERE name = '王小明'), 
   (SELECT id FROM "CREDIT_PACKAGE" WHERE name = '21 堂組合包方案'), 21, 4800),
...
VALUES 
  ((SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io'), 2, '教練資料描述'),
  ((SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io'), 2, '教練資料描述'),
  ((SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io'), 2, '教練資料描述');
...
SELECT c.id, s.id FROM "COACH" c, "SKILL" s
WHERE s.name = '重訓';

-- 肌肉棒子新增瑜伽專長
INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
SELECT c.id, s.id FROM "COACH" c, "SKILL" s
WHERE c.user_id = (SELECT id FROM "USER" WHERE name = '肌肉棒子') AND s.name = '瑜伽';

-- Q太郎新增有氧運動與復健訓練專長
INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
SELECT c.id, s.id FROM "COACH" c, "SKILL" s
WHERE c.user_id = (SELECT id FROM "USER" WHERE name = 'Q太郎') AND s.name IN ('有氧運動', '復健訓練');
...
INNER JOIN "CREDIT_PACKAGE" cp ON c.credit_package_id = cp.id
WHERE c.purchase_at BETWEEN '2024-11-01' AND '2024-11-30'
GROUP BY cp.name;
