...
UPDATE "USER" SET role = 'COACH' WHERE email IN ('lee2000@hexschooltest.io', 'muscle@hexschooltest.io', 'starplatinum@hexschooltest.io') AND role = 'USER';
...
(user_id, course_id, booking_at, status, join_at, leave_at, cancellation_reason, created_at) 
VALUES
(
...
inner join 
    (
        ...
        from "COURSE_BOOKING"
        where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') 
        and join_at is not null -- 確保已加入直播室
        and status != '課程已取消' -- 排除已取消的課程
        group by user_id
    )  as "COURSE_BOOKING"
on "COURSE_BOOKING".user_id = "CREDIT_PURCHASE".user_id;
...
where "SKILL".name = '重訓'  -- 直接使用專長名稱篩選
order by "COACH".experience_years desc;  -- 按經驗年數降序排列
...
select 
  ...
  count(*) as "銷售數量"
from "CREDIT_PURCHASE"
inner join "CREDIT_PACKAGE" cp ON "CREDIT_PURCHASE".credit_package_id = cp.id
